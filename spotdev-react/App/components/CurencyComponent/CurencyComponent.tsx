...
const CurencyComponent = ({firstNumber, secondNumber, result, operation, firstCurrency, secondCurrency} : CurrencyProps) => {
    ...
    //                 setData(postsData);
    //                 console.log(postsData.conversion_rates)
    //                 setError(null);
    //             } catch (err) {
            ...
            {loading ? (
                ...
                <div className={`${styles.container}`}>
                    <div className={`${styles.calculation}`}>
                        <span className={`${styles.number}`}>{firstNumber !== '' ? firstNumber : 0}</span>
                        <span className={`${styles.operator}`}>{operation}</span>
                        <span className={`${styles.number}`}>{secondNumber !== '' ? secondNumber : 0}</span>
                    </div>
                    <div className={`${styles.result}`}>
                        <span className={`${styles.converted}`}>
                        <span className={`${styles.amount}`}>{result}</span>
                        <span className={`${styles.currency} ${styles.gbp}`}>{firstCurrency}</span>
                        </span>
                        <span className={`${styles.converted}`}>
                        <span className={`${styles.amount} ${styles.large}`}>{convertedResult}</span>
                        <span className={`${styles.currency} ${styles.usd}`}>{secondCurrency}</span>
                        </span>
                    </div>
                </div>
            )} 
